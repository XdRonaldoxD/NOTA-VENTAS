...
export class ProductoInventarioService {
  ...
  ReporteNotaVenta(token,filtro):Observable<any>{
    ...
    return this.httpcliente.post(this.linkApi+'RerpoteProductoInventario',formData,{headers})
  }
  ListarInventarioProducto(token,filtro):Observable<any>{
    const formData=new FormData();
    formData.append('fechaInicio',filtro.fecha_inicio)
    formData.append('FechaFin',filtro.fecha_fin)
    const headers = new HttpHeaders({
    ...
    return this.httpcliente.post(this.linkApi+'ListarInventarioProducto',formData,{headers})
  }
}
